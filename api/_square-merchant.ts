...
export type SquareMerchantCredentials = { token: string; locationId: string; baseUrl: string }
...
async function serviceRequest(path: string) {
...
export async function getSquareMerchantCredentials(organizationId: string): Promise<SquareMerchantCredentials> {
  ...
  if (connectionResponse.ok && secretResponse.ok) {
    const connections = await connectionResponse.json() as Array<{ location_id?: string }>
    const secrets = await secretResponse.json() as Array<{ access_token_ciphertext?: string }>
